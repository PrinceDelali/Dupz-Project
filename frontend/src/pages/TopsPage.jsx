...
const TopsPage = () => {
  ...
  const fetchProducts = async () => {
    console.log('📝 TopsPage: Fetching TOPS products');
    setLoading(true);
    
    try {
      // First check if we have products in the store
      if (allProducts && allProducts.length > 0) {
        console.log(`📝 TopsPage: Found ${allProducts.length} products in store, filtering by category`);
        // Use the store's function to get TOPS products
        const categoryProducts = getProductsByCategory('TOPS');
        console.log(`📝 TopsPage: Found ${categoryProducts.length} products in TOPS category`);
        
        // Apply client-side sorting
        const sortedProducts = sortProducts(categoryProducts);
        setProducts(sortedProducts);
      } else {
        console.log('📝 TopsPage: No products in store, fetching from API');
        // Fetch all products first
        await fetchProductsFromAPI();
        
        // Then filter for TOPS
        const categoryProducts = getProductsByCategory('TOPS');
        console.log(`📝 TopsPage: Fetched ${categoryProducts.length} products in TOPS category from API`);
        
        // If still no products, try direct API call for just TOPS
        if (!categoryProducts || categoryProducts.length === 0) {
          console.log('📝 TopsPage: Making direct API call for TOPS category');
          ...
          if (response.data.success) {
            console.log(`📝 TopsPage: Received ${response.data.data.length} products directly from API`);
            ...
            console.error('📝 TopsPage: API call unsuccessful', response.data);
          }
        } else {
          // Apply client-side sorting
          const sortedProducts = sortProducts(categoryProducts);
  ...
  const getSortParam = () => {
    console.log(`📝 TopsPage: Getting sort parameter for ${sortBy}`);
    switch (sortBy) {
      case 'price-asc':
        return 'price';
      case 'price-desc':
  ...
  const sortProducts = (productArray) => {
    console.log(`📝 TopsPage: Sorting ${productArray.length} products by ${sortBy}`);
  ...
  const handleWishlistToggle = (e, product) => {
    ...
    if (!addToWishlist || !removeFromWishlist) {
      console.error('📝 TopsPage: Wishlist functions not available');
      return;
    }
      ...
      <div className="container mx-auto max-w-screen-xl px-4 py-8">
        <div className="flex flex-col space-y-4">
          <div className="flex justify-between items-center mb-6">
            <h1 className="text-3xl font-bold text-gray-900">Tops</h1>
