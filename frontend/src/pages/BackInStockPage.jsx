...
const BackInStockPage = () => {
  ...
  const fetchProducts = async () => {
    console.log('📝 BackInStockPage: Fetching BACK IN STOCK products');
    setLoading(true);
    
    try {
      // First check if we have products in the store
      if (allProducts && allProducts.length > 0) {
        console.log(`📝 BackInStockPage: Found ${allProducts.length} products in store, filtering by category`);
        // Use the store's function to get BACK IN STOCK products
        const categoryProducts = getProductsByCategory('BACK IN STOCK');
        console.log(`📝 BackInStockPage: Found ${categoryProducts.length} products in BACK IN STOCK category`);
        
        // Apply client-side sorting
        const sortedProducts = sortProducts(categoryProducts);
        setProducts(sortedProducts);
      } else {
        console.log('📝 BackInStockPage: No products in store, fetching from API');
        // Fetch all products first
        await fetchProductsFromAPI();
        
        // Then filter for BACK IN STOCK
        const categoryProducts = getProductsByCategory('BACK IN STOCK');
        console.log(`📝 BackInStockPage: Fetched ${categoryProducts.length} products in BACK IN STOCK category from API`);
        
        // If still no products, try direct API call for just BACK IN STOCK
        if (!categoryProducts || categoryProducts.length === 0) {
          console.log('📝 BackInStockPage: Making direct API call for BACK IN STOCK category');
          ...
          if (response.data.success) {
            console.log(`📝 BackInStockPage: Received ${response.data.data.length} products directly from API`);
            ...
            console.error('📝 BackInStockPage: API call unsuccessful', response.data);
          }
        } else {
          // Apply client-side sorting
          const sortedProducts = sortProducts(categoryProducts);
  ...
  const getSortParam = () => {
    console.log(`📝 BackInStockPage: Getting sort parameter for ${sortBy}`);
    switch (sortBy) {
      case 'price-asc':
        return 'price';
      case 'price-desc':
  ...
  const sortProducts = (productArray) => {
    console.log(`📝 BackInStockPage: Sorting ${productArray.length} products by ${sortBy}`);
  ...
  const handleWishlistToggle = (e, product) => {
    ...
    if (!addToWishlist || !removeFromWishlist) {
      console.error('📝 BackInStockPage: Wishlist functions not available');
      return;
    }
      ...
      <div className="container mx-auto max-w-screen-xl px-4 py-8">
        <div className="flex flex-col space-y-4">
          <div className="flex justify-between items-center mb-6">
            <h1 className="text-3xl font-bold text-gray-900">Back In Stock</h1>
