...
const BottomsPage = () => {
  ...
  const fetchProducts = async () => {
    console.log('📝 BottomsPage: Fetching BOTTOMS products');
    setLoading(true);
    
    try {
      // First check if we have products in the store
      if (allProducts && allProducts.length > 0) {
        console.log(`📝 BottomsPage: Found ${allProducts.length} products in store, filtering by category`);
        // Use the store's function to get BOTTOMS products
        const categoryProducts = getProductsByCategory('BOTTOMS');
        console.log(`📝 BottomsPage: Found ${categoryProducts.length} products in BOTTOMS category`);
        
        // Apply client-side sorting
        const sortedProducts = sortProducts(categoryProducts);
        setProducts(sortedProducts);
      } else {
        console.log('📝 BottomsPage: No products in store, fetching from API');
        // Fetch all products first
        await fetchProductsFromAPI();
        
        // Then filter for BOTTOMS
        const categoryProducts = getProductsByCategory('BOTTOMS');
        console.log(`📝 BottomsPage: Fetched ${categoryProducts.length} products in BOTTOMS category from API`);
        
        // If still no products, try direct API call for just BOTTOMS
        if (!categoryProducts || categoryProducts.length === 0) {
          console.log('📝 BottomsPage: Making direct API call for BOTTOMS category');
          ...
          if (response.data.success) {
            console.log(`📝 BottomsPage: Received ${response.data.data.length} products directly from API`);
            ...
            console.error('📝 BottomsPage: API call unsuccessful', response.data);
          }
        } else {
          // Apply client-side sorting
          const sortedProducts = sortProducts(categoryProducts);
      ...
      console.error('❌ Error fetching BOTTOMS products:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const getSortParam = () => {
    console.log(`📝 BottomsPage: Getting sort parameter for ${sortBy}`);
    switch (sortBy) {
      case 'price-asc':
        return 'price';
      case 'price-desc':
  ...
  const sortProducts = (productArray) => {
    console.log(`📝 BottomsPage: Sorting ${productArray.length} products by ${sortBy}`);
  ...
  const handleWishlistToggle = (e, product) => {
    ...
    if (!addToWishlist || !removeFromWishlist) {
      console.error('📝 BottomsPage: Wishlist functions not available');
      return;
    }
      ...
      <div className="container mx-auto max-w-screen-xl px-4 py-8">
        <div className="flex flex-col space-y-4">
          <div className="flex justify-between items-center mb-6">
            <h1 className="text-3xl font-bold text-gray-900">Bottoms</h1>
