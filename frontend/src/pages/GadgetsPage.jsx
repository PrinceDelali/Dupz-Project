...
const GadgetsPage = () => {
  ...
  const fetchProducts = async () => {
    console.log('📱 GadgetsPage: Fetching GADGETS products');
    setLoading(true);
    
    try {
      // First check if we have products in the store
      if (allProducts && allProducts.length > 0) {
        console.log(`📱 GadgetsPage: Found ${allProducts.length} products in store, filtering by category`);
        
        // Log all categories for debugging
        const availableCategories = [...new Set(allProducts.map(p => p.category))];
        console.log('📱 GadgetsPage: Available categories:', availableCategories);
        
        // Use case-insensitive filter instead of the store function
        const categoryProducts = allProducts.filter(product => 
          product.category && product.category.toUpperCase() === 'GADGETS'
        );
        
        console.log(`📱 GadgetsPage: Found ${categoryProducts.length} products in GADGETS category`);
        
        // Apply client-side sorting
        const sortedProducts = sortProducts(categoryProducts);
        setProducts(sortedProducts);
      } else {
        console.log('📱 GadgetsPage: No products in store, fetching from API');
        ...
        console.log(`📱 GadgetsPage: Fetched ${categoryProducts.length} products in GADGETS category from API`);
        
        // If still no products, try direct API call for just GADGETS
        if (categoryProducts.length === 0) {
          console.log('📱 GadgetsPage: Making direct API call for GADGETS category');
          const response = await axios.get(`${apiConfig.baseURL}/products`, {
            params: {
              category: 'GADGETS', // Keep uppercase for API call
              limit: 50,
              sort: getSortParam()
            }
          });
          ...
          if (response.data.success) {
            console.log(`📱 GadgetsPage: Received ${response.data.data.length} products directly from API`);
            const sortedProducts = sortProducts(response.data.data);
            setProducts(sortedProducts);
          } else {
            console.error('📱 GadgetsPage: API call unsuccessful', response.data);
          }
        } else {
          // Apply client-side sorting
          const sortedProducts = sortProducts(categoryProducts);
  ...
  const getSortParam = () => {
    console.log(`📱 GadgetsPage: Getting sort parameter for ${sortBy}`);
    switch (sortBy) {
      case 'price-asc':
        return 'price';
      case 'price-desc':
  ...
  const sortProducts = (productArray) => {
    console.log(`📱 GadgetsPage: Sorting ${productArray.length} products by ${sortBy}`);
  ...
  const handleWishlistToggle = (e, product) => {
    ...
    if (!addToWishlist || !removeFromWishlist) {
      console.error('📱 GadgetsPage: Wishlist functions not available');
      return;
    }
      ...
      <div className="container mx-auto max-w-screen-xl px-4 py-8">
        <div className="flex flex-col space-y-4">
          <div className="flex justify-between items-center mb-6">
            <h1 className="text-3xl font-bold text-gray-900">Gadgets</h1>
