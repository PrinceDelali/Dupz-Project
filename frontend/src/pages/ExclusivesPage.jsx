...
const ExclusivesPage = () => {
  ...
  const fetchProducts = async () => {
    console.log('✨ ExclusivesPage: Fetching EXCLUSIVES products');
    setLoading(true);
    
    try {
      // First check if we have products in the store
      if (allProducts && allProducts.length > 0) {
        console.log(`✨ ExclusivesPage: Found ${allProducts.length} products in store, filtering by category`);
        
        // Log all categories for debugging
        const availableCategories = [...new Set(allProducts.map(p => p.category))];
        console.log('✨ ExclusivesPage: Available categories:', availableCategories);
        
        // Use case-insensitive filter instead of the store function
        const categoryProducts = allProducts.filter(product => 
          product.category && product.category.toUpperCase() === 'EXCLUSIVES'
        );
        
        console.log(`✨ ExclusivesPage: Found ${categoryProducts.length} products in EXCLUSIVES category`);
        
        // Apply client-side sorting
        const sortedProducts = sortProducts(categoryProducts);
        setProducts(sortedProducts);
      } else {
        console.log('✨ ExclusivesPage: No products in store, fetching from API');
        ...
        console.log(`✨ ExclusivesPage: Fetched ${categoryProducts.length} products in EXCLUSIVES category from API`);
        
        // If still no products, try direct API call for just EXCLUSIVES
        if (categoryProducts.length === 0) {
          console.log('✨ ExclusivesPage: Making direct API call for EXCLUSIVES category');
          const response = await axios.get(`${apiConfig.baseURL}/products`, {
            params: {
              category: 'EXCLUSIVES', // Keep uppercase for API call
              limit: 50,
              sort: getSortParam()
            }
          });
          ...
          if (response.data.success) {
            console.log(`✨ ExclusivesPage: Received ${response.data.data.length} products directly from API`);
            const sortedProducts = sortProducts(response.data.data);
            setProducts(sortedProducts);
          } else {
            console.error('✨ ExclusivesPage: API call unsuccessful', response.data);
          }
        } else {
          // Apply client-side sorting
          const sortedProducts = sortProducts(categoryProducts);
      ...
      console.error('❌ Error fetching EXCLUSIVES products:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const getSortParam = () => {
    console.log(`✨ ExclusivesPage: Getting sort parameter for ${sortBy}`);
    switch (sortBy) {
      case 'price-asc':
        return 'price';
      case 'price-desc':
  ...
  const sortProducts = (productArray) => {
    console.log(`✨ ExclusivesPage: Sorting ${productArray.length} products by ${sortBy}`);
  ...
  const handleWishlistToggle = (e, product) => {
    ...
    if (!addToWishlist || !removeFromWishlist) {
      console.error('✨ ExclusivesPage: Wishlist functions not available');
      return;
    }
      ...
      <div className="container mx-auto max-w-screen-xl px-4 py-8">
        <div className="flex flex-col space-y-4">
          <div className="flex justify-between items-center mb-6">
            <h1 className="text-3xl font-bold text-gray-900">Exclusives</h1>
