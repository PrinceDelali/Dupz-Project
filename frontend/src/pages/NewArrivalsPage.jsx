...
const NewArrivalsPage = () => {
  ...
  const { success, cartNotification } = useToast();
  ...
  const fetchProducts = async () => {
    console.log('📝 NewArrivalsPage: Fetching NEW ARRIVALS products');
    setLoading(true);
    
    try {
      // First check if we have products in the store
      if (allProducts && allProducts.length > 0) {
        console.log(`📝 NewArrivalsPage: Found ${allProducts.length} products in store, filtering by category`);
        // Use the store's function to get NEW ARRIVALS products
        const categoryProducts = getProductsByCategory('NEW ARRIVALS');
        console.log(`📝 NewArrivalsPage: Found ${categoryProducts.length} products in NEW ARRIVALS category`);
        
        // Apply client-side sorting
        const sortedProducts = sortProducts(categoryProducts);
        setProducts(sortedProducts);
      } else {
        console.log('📝 NewArrivalsPage: No products in store, fetching from API');
        // Fetch all products first
        await fetchProductsFromAPI();
        
        // Then filter for NEW ARRIVALS
        const categoryProducts = getProductsByCategory('NEW ARRIVALS');
        console.log(`📝 NewArrivalsPage: Fetched ${categoryProducts.length} products in NEW ARRIVALS category from API`);
        
        // If still no products, try direct API call for just NEW ARRIVALS
        if (!categoryProducts || categoryProducts.length === 0) {
          console.log('📝 NewArrivalsPage: Making direct API call for NEW ARRIVALS category');
          ...
          if (response.data.success) {
            console.log(`📝 NewArrivalsPage: Received ${response.data.data.length} products directly from API`);
            ...
            console.error('📝 NewArrivalsPage: API call unsuccessful', response.data);
          }
        } else {
          // Apply client-side sorting
          const sortedProducts = sortProducts(categoryProducts);
  ...
  const getSortParam = () => {
    console.log(`📝 NewArrivalsPage: Getting sort parameter for ${sortBy}`);
    switch (sortBy) {
      case 'price-asc':
        return 'price';
      case 'price-desc':
  ...
  const sortProducts = (productArray) => {
    console.log(`📝 NewArrivalsPage: Sorting ${productArray.length} products by ${sortBy}`);
  ...
  const handleWishlistToggle = (e, product) => {
    ...
    if (!addToWishlist || !removeFromWishlist) {
      console.error('📝 NewArrivalsPage: Wishlist functions not available');
      return;
    }
