...
const DressesPage = () => {
  ...
  const fetchProducts = async () => {
    console.log('📝 DressesPage: Fetching DRESSES products');
    setLoading(true);
    
    try {
      // First check if we have products in the store
      if (allProducts && allProducts.length > 0) {
        console.log(`📝 DressesPage: Found ${allProducts.length} products in store, filtering by category`);
        // Use the store's function to get DRESSES products
        const categoryProducts = getProductsByCategory('DRESSES');
        console.log(`📝 DressesPage: Found ${categoryProducts.length} products in DRESSES category`);
        
        // Apply client-side sorting
        const sortedProducts = sortProducts(categoryProducts);
        setProducts(sortedProducts);
      } else {
        console.log('📝 DressesPage: No products in store, fetching from API');
        // Fetch all products first
        await fetchProductsFromAPI();
        
        // Then filter for DRESSES
        const categoryProducts = getProductsByCategory('DRESSES');
        console.log(`📝 DressesPage: Fetched ${categoryProducts.length} products in DRESSES category from API`);
        
        // If still no products, try direct API call for just DRESSES
        if (!categoryProducts || categoryProducts.length === 0) {
          console.log('📝 DressesPage: Making direct API call for DRESSES category');
          ...
          if (response.data.success) {
            console.log(`📝 DressesPage: Received ${response.data.data.length} products directly from API`);
            ...
            console.error('📝 DressesPage: API call unsuccessful', response.data);
          }
        } else {
          // Apply client-side sorting
          const sortedProducts = sortProducts(categoryProducts);
      ...
      console.error('❌ Error fetching DRESSES products:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const getSortParam = () => {
    console.log(`📝 DressesPage: Getting sort parameter for ${sortBy}`);
    switch (sortBy) {
      case 'price-asc':
        return 'price';
      case 'price-desc':
  ...
  const sortProducts = (productArray) => {
    console.log(`📝 DressesPage: Sorting ${productArray.length} products by ${sortBy}`);
  ...
  const handleWishlistToggle = (e, product) => {
    ...
    if (!addToWishlist || !removeFromWishlist) {
      console.error('📝 DressesPage: Wishlist functions not available');
      return;
    }
      ...
      <div className="container mx-auto max-w-screen-xl px-4 py-8">
        <div className="flex flex-col space-y-4">
          <div className="flex justify-between items-center mb-6">
            <h1 className="text-3xl font-bold text-gray-900">Dresses</h1>
