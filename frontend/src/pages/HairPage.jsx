...
const HairPage = () => {
  ...
  const fetchProducts = async () => {
    console.log('💇 HairPage: Fetching HAIR products');
    setLoading(true);
    
    try {
      // First check if we have products in the store
      if (allProducts && allProducts.length > 0) {
        console.log(`💇 HairPage: Found ${allProducts.length} products in store, filtering by category`);
        
        // Log all categories for debugging
        const availableCategories = [...new Set(allProducts.map(p => p.category))];
        console.log('💇 HairPage: Available categories:', availableCategories);
        
        // Use case-insensitive filter instead of the store function
        const categoryProducts = allProducts.filter(product => 
          product.category && product.category.toUpperCase() === 'HAIR'
        );
        
        console.log(`💇 HairPage: Found ${categoryProducts.length} products in HAIR category`);
        
        // Apply client-side sorting
        const sortedProducts = sortProducts(categoryProducts);
        setProducts(sortedProducts);
      } else {
        console.log('💇 HairPage: No products in store, fetching from API');
        ...
        console.log(`💇 HairPage: Fetched ${categoryProducts.length} products in HAIR category from API`);
        
        // If still no products, try direct API call for just HAIR
        if (categoryProducts.length === 0) {
          console.log('💇 HairPage: Making direct API call for HAIR category');
          const response = await axios.get(`${apiConfig.baseURL}/products`, {
            params: {
              category: 'HAIR', // Keep uppercase for API call
              limit: 50,
              sort: getSortParam()
            }
          });
          ...
          if (response.data.success) {
            console.log(`💇 HairPage: Received ${response.data.data.length} products directly from API`);
            const sortedProducts = sortProducts(response.data.data);
            setProducts(sortedProducts);
          } else {
            console.error('💇 HairPage: API call unsuccessful', response.data);
          }
        } else {
          // Apply client-side sorting
          const sortedProducts = sortProducts(categoryProducts);
  ...
  const getSortParam = () => {
    console.log(`💇 HairPage: Getting sort parameter for ${sortBy}`);
    switch (sortBy) {
      case 'price-asc':
        return 'price';
      case 'price-desc':
  ...
  const sortProducts = (productArray) => {
    console.log(`💇 HairPage: Sorting ${productArray.length} products by ${sortBy}`);
  ...
  const handleWishlistToggle = (e, product) => {
    ...
    if (!addToWishlist || !removeFromWishlist) {
      console.error('💇 HairPage: Wishlist functions not available');
      return;
    }
